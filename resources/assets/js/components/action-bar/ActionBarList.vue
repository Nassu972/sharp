<template>
    <sharp-action-bar class="SharpActionBarList" :class="{'SharpActionBarList--search-active':searchActive}" :ready="ready">
        ...
        <template slot="left">
            <span class="text-content">{{ resultsCount }} {{ l('action_bar.list.items_count') }}</span>
        </template>
        <template slot="right">
    ...
    export default {
        ...
        computed: {
            ...mapState('entity-list', {
                resultsSearch: state => state.results.search,
                forms: state => state.config.forms,
                sortActive: state => state.sortActive,
            }),
            ...
            filterKey(filter) {
                return `actionbarlist_${filter.key}`;
            },
        },
        methods: {
            handleSearchFocused() {
